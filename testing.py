...
if len(filtered_df) >= 20:
    ...
    X = reg_df[['log_AvgUnitPrice', 'Promo', 'log_Stores']].values
    y = reg_df['log_Units'].values
    # Add constant term (intercept)
    X_const = np.column_stack((np.ones(len(X)), X))
    
    # Compute coefficients using least squares
    coeff, residuals, rank, s = np.linalg.lstsq(X_const, y, rcond=None)
    const = coeff[0]
    beta_price = coeff[1]
    beta_promo = coeff[2]
    beta_stores = coeff[3]
    ...
    y_pred = X_const.dot(coeff)
    # Compute R-squared for model accuracy
